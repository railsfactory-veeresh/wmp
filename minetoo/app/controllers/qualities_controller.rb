class QualitiesController < ApplicationController
  ...
  def create
    @quality = Quality.new(quality_params)
     @quality.user_id=current_user.id

    respond_to do |format|
      if @quality.save
        format.html { redirect_to root_path, notice: 'Quality was successfully created.' }
  ...
  private
    ...
    def quality_params
      params.require(:quality).permit(:conisiderate, :boisterous, :wise, :relaxed, :forward_looking, :intense, :indecisive, :strong, :grounded, :emotional, :demonstrative, :wity, :unaffected, :inscrutable, :mercurial, :user_id)
    end
end
